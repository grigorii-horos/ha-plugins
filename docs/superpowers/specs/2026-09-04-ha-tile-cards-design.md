...
Пятнадцать карточек. Четырнадцать — плитки: одна строка, роли, правая колонка значений.
...
**Приёмка — собранный бандл в `/config/www/`**, ресурс переключается на
`/local/ha-plugins-cards.js?v=N`. Версия в query обязательна, иначе HA отдаёт
закешированный файл. Требует доступа к файловой системе HA, которого у сборки нет.
...
Скрипт разворачивания идемпотентный: повторный запуск не плодит дубликаты, а обновляет
конфиг и переставляет ресурс при смене порта.
